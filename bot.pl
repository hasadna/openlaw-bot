...
if ((%new_pages) && !($onlycheck || $dryrun)) {
	...
	if ($text) {
		...
		foreach my $p (keys(%new_pages)) {
			if ($p =~ /^(חוק|פקודת)/) {
				$new .= "[[$p]] {{*}}\n";
			} elsif ($text =~ /<!-- בוט שורה (?:אחת|ראשונה) -->/) {
				$text =~ s/^(.*?) *<!-- בוט שורה (?:אחת|ראשונה) -->.*$/[[$p]] <!-- בוט שורה ראשונה --> {{*}}/m;
				my $second = $1 // '';
				if ($text =~ /<!-- בוט שורה (?:שתיים|שנייה)-->/) {
					$text =~ s/^.*<!-- בוט שורה (?:שתיים|שנייה)-->.*$/$second <!-- בוט שורה שנייה --> {{*}}/m;
				} else {
					$text =~ s/(<!-- בוט שורה (?:אחת|ראשונה) -->.*)\n/$1\n$second <!-- בוט שורה שנייה --> {{*}}\n/;
				}
			} else {
