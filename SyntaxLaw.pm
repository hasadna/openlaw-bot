...
our $extref_sig = $pre_sig . '(חוק(?: ה?יסוד:?|)|פקוד[הת]|תקנות|צו|החלטה|הכרזה|תקנון|הוראו?ת|הודעה|מנשר|כללים?|נוהל|קביעו?ת|חוק[הת]|אמנ[הת]|דברי?[ -]ה?מלך|החלטו?ת|הנחי[יו]ת|קווים מנחים)';
...
sub parse_line {
	...
	$str .= ">";
	$str .= "<מספר>$num$bold</מספר> " if ($num);
	$str .= "$bold$line" if (length($line)>0);
	$str .= "</$type>\n";
	return $str;
}
