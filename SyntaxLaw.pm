...
our $extref_sig = $pre_sig . '(חוק(?:[ -]ה?יסוד:?|)|פקוד[הת]|תקנות(?: שעת[ -]ה?חי?רום)?|צו|(?:החלט|הכרז|אכרז|הודע)(?:ה|ו?ת)|תקנון|הוראו?ת|רשימת|מנשר|כללים?|נוהל|רי?שיון|קביעו?ת|נורמו?ת|חוק[הת]|אמנ[הת]|דברי?[ -]ה?מלך|הנחי[יו]ת|קווים מנחים|אמות מידה|היתר|(?:כתבי? )?מי?נוי)';
...
sub find_href {
	...
	if (/^HTTPS?:/) { $_ = lc($_); }
	if (/^(https?:|mailto:)/) {
		s/%([Dd][0-9A-Fa-f])%([0-9A-Fa-f]{2})/decode_utf8(chr(hex($1)).chr(hex($2)))/eg;
		return ('', $_);
	}
